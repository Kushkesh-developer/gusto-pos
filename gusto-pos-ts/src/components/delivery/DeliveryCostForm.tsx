...
const generateZodSchema=(translate:TranslateFn)=>{
    return z.object({
        ...
        priceAfterEveryKm:z.string().min(1,translate("please_enter_price_every_km")),
        freeDeliveryAt:z.string().min(1,translate("Free_delivery_must_be_$40")),
        minimumPurchase:z.string().min(1,translate("minimum_purchase_must_be_$5")),
        feeForFallingBelowMinimumPurchase:z.string().min(1,translate("please_enter_above_$2_otherWise_charge_may_apply")),
    })
}
     ...
     return(
        <Box sx={{maxWidth:"1140px"}}>
            <form onSubmit={handleSubmit(onSubmit)}>
              <Box mb={5}>
              <FormLayout cardHeading={translate("logistics_company")} showSwitch={true}>
                        <Controller
                            name="startingPrice"
                            control={control}
                            render={({ field }) => (
