...
// Import DaySelector component
import DaySelector from "../widgets/inputs/GSDaySelector"; // Update the path accordingly
...
interface FormData {
  ...
  avgSpend: string;
  note: string;
  selectedDays: string[]; // Include selectedDays field
}
...
const CustomerForm = () => {
  ...
  const {
    ...
    defaultValues: {
      ...
      avgSpend: "",
      note: "",
      selectedDays: [] // Default for selected days
    },
  });
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Box mb={5}>
          <FormLayout cardHeading={translate("customer_details")}>
            ...
            {/* Add DaySelector Here */}
            <Box mt={3} withoutGrid>
              <DaySelector selectedDays={selectedDays} onChange={handleDayChange} />
              {errors.selectedDays && (
                <span style={{ color: "red" }}>{errors.selectedDays.message}</span>
              )}
            </Box>
            {/* End of DaySelector */}
