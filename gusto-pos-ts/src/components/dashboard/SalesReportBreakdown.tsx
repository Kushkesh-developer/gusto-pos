...
export default function SalesReportBreakdown({ stalesBreakDownReportData = [] }: { stalesBreakDownReportData: SalesBreakdownsReportType[] }) {
                    ...
                    return (
                        <Stack mt={3}>
                            ...
                            <Stack key={data.title} direction={"row"} alignItems={"center"}>
                                ...
                                {data.quantityHeading && <Typography sx={{fontWeight:"500"}}>{data.quantityHeading}</Typography>}
                                {data.amountHeading && <Typography sx={{ minWidth: 100, fontWeight:"500"}} textAlign={"end"}>{data.amountHeading}</Typography>}
                            </Stack>
